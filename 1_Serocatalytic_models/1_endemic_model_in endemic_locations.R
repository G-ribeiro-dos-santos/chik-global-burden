...
removed_areas = c("Odisha","West Bengal","Bihar","Tripura","Meghalaya","Assam","Punjab", #Remove places in India with no circulation
                  "Kitui", "Central Nyanza", "Kainji Lakes") #Remove old serostudies

table(data$continent, data$classification)
...
for(subset in list_subsets){
  select_vect =  data$classification=="Endemic" & !colnames(data$N_pos) %in% removed_areas & data$continent %in% subset
  age_max = 30
  ...
  output <- mod$sample(data=data_sub, chains=4, parallel_chains=4, iter_sampling=4000, refresh=100, iter_warmup=500, init=rep(list(list_init),4))
  output <- rstan::read_stan_csv(output$output_files())
  chains<-rstan::extract(output, inc_warmup=FALSE)
  save(chains, output, file=paste0("1_Serocatalytic_models/chains_endemic_hyperprior_",paste0(subset,collapse=""),".Rdata"))
}


## Fet endemic FOI
chains = lapply(list_subsets, function(subset){
  load(paste0("1_Serocatalytic_models/chains_endemic_hyperprior_",paste0(subset,collapse=""),".Rdata"))
  chains})
...
write.csv(lambda, file="1_Serocatalytic_models/estimates_endemic_in_endemic.csv", row.names = F)
ggsave(p_paper, file="1_Serocatalytic_models/sensitivity_continent_estimates_mcmc_endemic.pdf")
